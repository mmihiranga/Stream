...
function HomeScreen({navigation}) {
  ...
  const [userDetails, setUserDetails] = useState([
    {cardDetails: {amount: '00.00',cardNo:'----------------',expiryDate:'----/----/--'} },
  ]);
    ...
    if (userData.id) {
      api
        ...
        .then(res => {
          ...
          if(res.data[0].cardDetails.cardNo !=null){
  ...
  const handleUserOut = () => {
    if (active == true) {
      ...
      if (trip != null) {
        ...
        if (UID._id) {
          recharge = parseFloat(UID.cardDetails.amount) - parseFloat(tot);
          const payment = {
            id: UID._id,
            cardDetails: {
          ...
          api
            ...
            .then(function (response) {
              ...
              api.post('payment/create',paymentC).then(function (response) {
                if (response.data.message) {
                  console.log(response.data.message);
                }
  ...
  const handleRecharge=()=>{
    ...
    if(UID.cardDetails.cardNo ==null){
      ToastAndroid.showWithGravityAndOffset(
      ...
      navigation.push('Recharge', {userID: UID})
    }
    ...
    <View
      ...
      <View style={styles.SummaryView}>
        <View style={styles.amountViewFlex}>
          ...
          <View>
            <TransactionModal userID={userDetails[0]._id}/>
          </View>
        </View>
      ...
      <View style={styles.UserCardView}>
        ...
        <Text style={styles.cardInsideNo}>
            {userDetails[0].cardDetails.cardNo.substring(0, 4) + ' '}
            {userDetails[0].cardDetails.cardNo.substring(4, 8) + ' '}
            {userDetails[0].cardDetails.cardNo.substring(8, 12) + ' '}
            {userDetails[0].cardDetails.cardNo.substring(12, 16) + ' '}
        </Text>
        <Text style={styles.cardInsideDate}>{userDetails[0].cardDetails.expiryDate.substring(2, 5)}{userDetails[0].cardDetails.expiryDate.substring(7, 9)}</Text>
